...
class Printer:
        ...
        if self.state.stage == PrinterStage.PAUSED and prev_stage != PrinterStage.PAUSED:
            await self.bus.publish(
                PauseEvent(
                    ...
                    # PHASE-0: verify — whether the report carries a layer/tag for richer
                    # validation (docs/10 #3/#29). None until confirmed.
    ...
    async def routine_unload(self) -> None:
        ...
        PHASE-0: verify — payload built by the driver is a guess until v0.2 (docs/09 #3).
        """
    ...
    async def routine_extrude(self) -> None:
        ...
        PHASE-0: verify — driver payload unconfirmed (docs/09 #3).
        """
    ...
    async def routine_confirm_resume(self) -> None:
        ...
        PHASE-0: verify — if this verb is wrong the print sits paused forever (docs/09 #3).
        """
    ...
    async def send_job(self, file: str) -> str:
        ...
        return await self.ftp.upload(self.id, file)

    async def start_print(self, path: str) -> None:
        """Start printing the uploaded file at ``path``.

        PHASE-0: verify — start-print command verb + path form (driver; docs/03).
        """
        await self.link.request(self.driver.request_start_print(path))
