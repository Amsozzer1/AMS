...
class FtpsClient:
    ...
    The printer exposes an implicit-FTPS server on port 990; auth is user ``bblp`` with the
    printer's access code, and the TLS cert is self-signed (we don't verify it — the link is
    LAN-local and access-code-gated). ``ftplib`` is blocking, so the actual transfer runs in a
    worker thread. The driver wraps the returned name as ``ftp://<name>`` for ``start_print``.

    PHASE-0 (v0.2 spike) — implemented from the documented Bambu pattern but **not yet
    confirmed against a real printer**:

    * that login lands at a writable root and ``STOR <name>`` is accepted there (vs needing
      ``/cache/`` or another dir),
    * that ``start_print``'s ``url``/``param`` accept the resulting ``ftp://<name>`` form
      (docs/03-tech-stack.md FTPS; docs/09 "Still UNVERIFIED").

    When the spike confirms these, drop this note and the matching driver ``# PHASE-0: verify``
    comments — never mark them confirmed without a real printer.
    """
    ...
    async def upload(self, printer: PrinterId, file: str) -> str:
        """Upload ``file`` to the printer and return the remote name ``start_print`` references."""
        name = file.rsplit("/", 1)[-1]
        await asyncio.to_thread(self._upload_blocking, file, name)
        # Log the name only — never the access code.
        log.info("FtpsClient: uploaded %s to printer %s", name, printer)
        return name
